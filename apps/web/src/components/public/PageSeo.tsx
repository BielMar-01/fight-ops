...
export function PageSeo({ seo }: PageSeoProps) {
  useEffect(() => {
    ...
    setMetaTag('meta[property="og:title"]', 'content', seo.ogTitle ?? seo.title)

    setMetaTag('meta[property="og:description"]', 'content', seo.ogDescription ?? seo.description)

    if (seo.ogImageUrl) {
      setMetaTag('meta[property="og:image"]', 'content', seo.ogImageUrl)
    }
