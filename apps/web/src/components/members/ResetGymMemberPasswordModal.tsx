...
import { resetGymMemberPassword } from '../../services/gym-member.service'
...
export function ResetGymMemberPasswordModal({
  ...
  }, [isSubmitting, success, onBack, onCompleted])
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await resetGymMemberPassword(gymId, member.id, {
        password,
        confirmPassword,
      })
      ...
      setError('Não foi possível redefinir a senha do usuário.')
    } finally {
        ...
        >
          <header className="member-modal-header">
            <div>
              ...
              <h2 id="member-password-reset-success-title">Senha redefinida</h2>

              <p>A nova senha foi configurada para {member.user.name}.</p>
            </div>
          ...
          <div className="member-password-success">
            <div className="member-password-success-icon" aria-hidden="true">
              ✓
            </div>
            ...
            <div>
              ...
              <p>
                As sessões anteriores desse usuário foram encerradas. Ele deverá utilizar a nova
                senha para entrar novamente no FightOps.
              </p>
            </div>
          </div>

          <div className="member-password-security-note">
            <strong>Registro de segurança</strong>

            <p>
              Essa operação foi registrada na Auditoria da academia. A senha não é armazenada no
              histórico.
            </p>
          </div>
      ...
      >
        <header className="member-modal-header">
          <div>
            ...
            <h2 id="member-password-reset-title">Redefinir senha</h2>

            <p>Defina uma nova senha para {member.user.name}.</p>
          </div>
        ...
        <form className="member-modal-form" onSubmit={handleSubmit}>
          ...
          <div className="member-password-warning">
            ...
            <p>Após a redefinição, todas as sessões atuais deste usuário serão encerradas.</p>
          </div>
          ...
          {error ? (
            <div className="form-error" role="alert" data-testid="member-password-reset-error">
              {error}
            </div>
          ) : null}
