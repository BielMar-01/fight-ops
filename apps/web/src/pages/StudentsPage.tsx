...
import type { Student, StudentPagination } from '../types/student'
...
export function StudentsPage() {
  ...
  const [pagination, setPagination] = useState<StudentPagination>(initialPagination)
  ...
  const canManageStudents =
    activeGym?.role === 'OWNER' || activeGym?.role === 'ADMIN' || activeGym?.role === 'RECEPTIONIST'
  ...
  const loadStudents = useCallback(async () => {
    ...
    try {
      ...
      const response = await getStudents(activeGym.id, {
        page,
  ...
  if (activeGym && !canViewStudents) {
    return (
      <section className="students-page" data-testid="students-page">
        <div className="students-access-denied" data-testid="students-access-denied">
          <div className="students-access-denied-icon">!</div>

          <span className="students-eyebrow">Acesso restrito</span>

          <h1>Área não disponível</h1>

          <p>Seu perfil nesta academia não possui acesso à gestão de alunos.</p>
        </div>
      </section>
    )
  }
  ...
  return (
    <>
      <section className="students-page" data-testid="students-page">
        <header className="students-header">
          <div>
            ...
            <p>Consulte e gerencie os alunos cadastrados na academia selecionada.</p>
          </div>

          <div className="students-header-actions">
            <div className="students-total" data-testid="students-total">
              <strong>{pagination.total}</strong>

              <span>{pagination.total === 1 ? 'aluno' : 'alunos'}</span>
            </div>
        ...
        <div className="students-filters" data-testid="students-filters">
        ...
        {loading ? (
          <div className="students-state" data-testid="students-loading">
            <div className="students-loading-spinner" aria-hidden="true" />

            <span>Carregando alunos...</span>
          </div>
        ) : error ? (
          <div className="students-state students-state-error" data-testid="students-error">
          ...
          <div className="students-empty" data-testid="students-empty">
          ...
          <>
            <div className="students-list" data-testid="students-list">
            ...
            <div className="students-pagination" data-testid="students-pagination">
              <button
                type="button"
                className="students-button students-button-secondary"
              ...
              <span className="students-pagination-info" data-testid="students-pagination-info">
      ...
      {isAddModalOpen && activeGym && canManageStudents ? (
        <AddStudentModal
          gymId={activeGym.id}
          onClose={() => {
            setIsAddModalOpen(false)
      ...
      {selectedStudentId && activeGym && canViewStudents ? (
        <ManageStudentModal
          gymId={activeGym.id}
