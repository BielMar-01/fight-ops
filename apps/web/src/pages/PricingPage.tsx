...
export function PricingPage() {
  const { data, isLoading, isError, refetch } = usePublicPage('pricing')
  ...
  const plans = page.sections.find((section) => section.key === 'plans')

  const metadata = plans?.metadata as PricingMetadata | null
    ...
    <main>
      ...
      <section className="section">
        <div className="site-container pricing-grid">
          {metadata?.items?.map((plan) => (
            <article
              className={plan.featured ? 'pricing-card featured' : 'pricing-card'}
              key={plan.key}
            >
              {plan.featured ? <span className="pricing-badge">Mais completo</span> : null}
