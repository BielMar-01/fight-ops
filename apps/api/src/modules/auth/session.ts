...
export async function rotateSession(refreshToken: string) {
  ...
  if (!session) {
    throw new AppError('INVALID_REFRESH_TOKEN', 401, 'Sessão inválida ou expirada.')
  }

  if (session.revokedAt) {
    throw new AppError('SESSION_REVOKED', 401, 'Sessão revogada.')
  }

  if (session.expiresAt <= new Date()) {
    throw new AppError('SESSION_EXPIRED', 401, 'Sessão expirada.')
  }
