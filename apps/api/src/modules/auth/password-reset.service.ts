...
function getCodeExpirationDate() {
  ...
  expiresAt.setMinutes(expiresAt.getMinutes() + env.PASSWORD_RESET_CODE_EXPIRATION_MINUTES)
...
export async function verifyPasswordResetCode(input: VerifyPasswordResetInput) {
  ...
  if (!user) {
    throw new AppError('INVALID_RESET_CODE', 400, 'Código inválido ou expirado.')
  }
...
export async function resetPassword(input: ResetPasswordInput) {
  ...
  if (
    ...
    throw new AppError('INVALID_RESET_TOKEN', 400, 'Token de redefinição inválido ou expirado.')
  }
